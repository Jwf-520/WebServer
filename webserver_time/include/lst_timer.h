...
public:
    ...
    void (*cb_func)(http_conn1* user_data); //任务回调函数，回调函数处理的客户数据，由定时器的执行者传递给回调函数
    http_conn1* user_data;
...
public:
    ...
    void add_timer(util_timer* timer) {
        ...
        if (!head) { //头结点是空，就把头尾节点编程该定时器
            head = tail = timer;
            return;
        }
        /*
            如果目标定时器的超时时间小于当前链表所有定时器的超时时间，则把该定时器插入链表头部，作为链表新的头结点，
            否则则需要调用重载函数add_timer(), 把它插入链表中合适的位置。以保证链表的升序特性
        */
    ...
    */
    ...
    void tick() {
        ...
        while (tmp) {
            /*因为每个定时器都使用绝对时间作为超时值，所以可以把定时器的绝对值和系统当前时间，
            比较以判断定时器是否到期*/
            if (cur < tmp->expire) {
                break;
            }
            //调用定时器的回调函数，它删除非活动连接socket上的注册事件，并关闭之。
